...
Независимо от фактической категории итератора, заданной при помощи вложенного объявления типа `iterator_category`, на данный момент поддерживаются только операции, соответствующие категориям `InputIterator` и `ForwardIterator`.
