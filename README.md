...
1. Не поддерживаются операторы постинкремента и постдекремента `it++`, `it--`.
...
1. Не поддерживается обмен разыменованных итераторов при помощи функции swap: `swap(*it1, *it2)`.
   В частности, это означает, что пару `ZipIterator` нельзя использовать в алгоритме `std::sort`.
   Однако для самих итераторов функция swap предоставляется.
